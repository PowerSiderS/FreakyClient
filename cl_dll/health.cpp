...
void CHudHealth::DrawHealthBar( float flTime )
{
        ...
        if (cl_health_transition && cl_health_transition->value > 0)
        {
                float flTarget = (float)m_iHealth;
                float flSpeed = cl_health_transition_speed ? cl_health_transition_speed->value : 10.0f;
                ...
                {
                        m_flDisplayHealth += flStep;
                }
                
                if (m_flDisplayHealth < 0) m_flDisplayHealth = 0;
        }
        else
        {
                m_flDisplayHealth = (float)m_iHealth;
